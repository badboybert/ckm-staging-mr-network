...
def add_inline(p, text):
    ...
    for m in INLINE.finditer(text):
        ...
        if m.group(1) is not None:
            ...
            # ^...^ -> superscript run (affiliation markers on the title page; R6 M7)
            p.add_run(m.group(4)).font.superscript = True
        pos = m.end()
    if pos < len(text):
        p.add_run(text[pos:])
...
_ADDRESS_CUES = ("on behalf of all authors", "Email:", "Correspondence:", "To the Editors",
                 "ORCID ", "Additional affiliations:")

def _is_address_block(buf):
    """An address/signature block: short lines, at least one carrying an address cue, and no line
    long enough to be prose. Kept deliberately narrow so ordinary wrapped paragraphs still join."""
    if any(len(x) > 120 for x in buf):
        return False
    return sum(any(c in x for c in _ADDRESS_CUES) for x in buf) >= 1
...
def render_md(doc, md):
    ...
    while i < len(lines):
        ...
        # Round-7 C108: joining is right for prose and wrong for ADDRESS blocks. A block whose lines
        # are an addressee, a signature or a correspondence address is emitted with real line breaks,
        # so the cover letter stops reading "Bertrand Chin-Ming Tan, PhD on behalf of all authors
        # Department of ... Taoyuan City 33302, Taiwan Email: ..." as one run-on line.
        if len(buf) > 1 and _is_address_block(buf):
            p = doc.add_paragraph()
            for _k, _ln in enumerate(buf):
                if _k:
                    p.add_run().add_break()
                add_inline(p, _ln)
        else:
...
for f in ["INTRODUCTION.md", "METHODS.md", "RESULTS.md", "DISCUSSION.md",
          ...
          # Round-7 Q7: Table 1 (graded evidence synthesis), generated by scripts/build_tables.py.
          "TABLES.md",
...
- [ ] Confirm TPMI and KoGES data-access wording (DECLARATIONS.docx; Supplementary Table S1).
      TPMI is now cited via its cohort-descriptor paper (Yang HC et al., Nature 2025;648:117-127) added
      in round 6; the data-access wording itself is unchanged.
...
list: the code
repository URL and archived DOI, and the data-access wording for the East Asian cohorts.
