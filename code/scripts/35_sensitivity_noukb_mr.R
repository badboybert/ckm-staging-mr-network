...
# ------------------------------------------------------------------------------------------------
# Step 35b (UKB-free OUTCOME sensitivity): harmonise + MR battery for every noUKB edge.
# Mirrors 06_mr.R exactly (harmonise action=2, mr_keep, >=3 SNPs; IVW/Egger/WM + pleiotropy,
# heterogeneity, directionality). Reads data/harmonised_noukb/<EXP>__<O>_<PROVIDER>.outcome.tsv.
# Run (PowerShell): & "C:/Program Files/R/R-4.4.1/bin/Rscript.exe" scripts/35_sensitivity_noukb_mr.R
suppressMessages({library(TwoSampleMR)})
BASE  <- P4_BASE
INSTR <- file.path(BASE,"data/instruments"); HARM <- file.path(BASE,"data/harmonised_noukb")
...
rows <- list()
for (of in ofiles) {
  base  <- sub("\\.outcome\\.tsv$","",basename(of))          # <EXP>__<O>_<PROVIDER>
  parts <- strsplit(base,"__")[[1]]; exp <- parts[1]; rest <- parts[2]
  # rest = <O>_<PROVIDER>; neither token contains '_'
  rp <- strsplit(rest,"_")[[1]]; out <- rp[1]; prov <- paste(rp[-1], collapse="_")
  ef <- file.path(INSTR, paste0(exp,".clumped.tsv"))
  if (!file.exists(ef)) next
  e <- tryCatch(fmt_exp(ef, exp), error=function(z) NULL)
  o <- tryCatch(fmt_out(of),      error=function(z) NULL)
  if (is.null(e)||is.null(o)||nrow(o)==0) { cat(sprintf("%-6s -> %-6s [%s] : no data\n",exp,out,prov)); next }
  h <- harmonise_data(e, o, action=2)
  h <- h[h$mr_keep,]
  if (nrow(h)<3) { cat(sprintf("%-6s -> %-6s [%s] : <3 SNPs\n",exp,out,prov)); next }
  ...
  rows[[base]] <- data.frame(
    exposure=exp, outcome=out, provider=prov, nsnp=gv(m,"Inverse variance weighted","nsnp"),
  ...
  cat(sprintf("%-6s -> %-6s [%-10s]: nSNP=%3d  IVW b=%+.4f p=%.2e  Steiger=%s\n",
      exp,out,prov, rows[[base]]$nsnp, rows[[base]]$ivw_b, rows[[base]]$ivw_p, rows[[base]]$steiger_correct))
}
res <- do.call(rbind, rows)
res <- res[order(res$outcome, res$exposure, res$provider),]
write.csv(res, file.path(RES,"sensitivity_noukb_edges.csv"), row.names=FALSE)
cat("\nWrote", file.path(RES,"sensitivity_noukb_edges.csv"), "with", nrow(res), "edges\n")
