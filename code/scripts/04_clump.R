...
# ------------------------------------------------------------------------------------------------
# Step 4: LD-clump genome-wide-sig instruments locally (PLINK + 1000G EUR).
# Usage (PowerShell): Rscript 04_clump.R LDL CAD HF T2D eGFR
suppressMessages({library(ieugwasr); library(dplyr)})
BASE <- P4_BASE
PLINK <- file.path(CKM_ROOT, "lhcmr_eas_refs", "tools", "plink.exe")
BFILE <- file.path(CKM_ROOT, "paper 6", "analysis", "g1000_eur", "g1000_eur")

args <- commandArgs(trailingOnly=TRUE); if(!length(args)) args <- c("LDL","CAD","HF","T2D","eGFR")
for (node in args) {
  f <- file.path(BASE,"data/instruments",paste0(node,".sig.tsv"))
  ...
  cl <- tryCatch(
    ieugwasr::ld_clump(d2, plink_bin=PLINK, bfile=BFILE,
                       clump_r2=0.001, clump_kb=10000, clump_p=1),
    error=function(e){cat(node,"CLUMP ERROR:",conditionMessage(e),"\n"); NULL})
  if (is.null(cl)) next
  out <- d[d$SNP %in% cl$rsid, ]
  outf <- file.path(BASE,"data/instruments",paste0(node,".clumped.tsv"))
  write.table(out, outf, sep="\t", row.names=FALSE, quote=FALSE)
  cat(sprintf("[clump] %-6s %6d sig -> %4d independent instruments -> %s\n",
