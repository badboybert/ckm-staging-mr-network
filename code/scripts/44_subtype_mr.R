...
# ------------------------------------------------------------------------------------------------
# Step 44: MR for the HF-subtype edges (RF -> niHF/niHFpEF/niHFrEF), same estimators as 06_mr.R.
# Reads <exp>.clumped.tsv + harmonised_hfsub/<exp>__<sub>.outcome.tsv. Run: Rscript 44_subtype_mr.R
...
for (of in ofiles) {
  ...
  if (nrow(h) < 3) { cat(sprintf("%-6s -> %-8s : <3 SNPs\n", exp, out)); next }
  ...
  cat(sprintf("%-6s -> %-8s : nSNP=%3d  IVW b=%+.4f p=%.2e\n",
      exp, out, rows[[base]]$nsnp, rows[[base]]$ivw_b, rows[[base]]$ivw_p))
}
res <- do.call(rbind, rows)
write.csv(res, file.path(RES, "network_hfsubtypes.csv"), row.names=FALSE)
cat("\nWrote results/network_hfsubtypes.csv with", nrow(res), "subtype edges\n")
