...
# 2026-09-17: the interim statement R6 asked for was a stand-in until the deposit existed. v1.2.0 is now
# archived, so asserting the interim wording would ENFORCE a stale placeholder. The truthful statement
# is now the real identifiers, read from manifest/DEPOSIT.json (concept DOI, never the version DOI).
import json as _json
_dep = _json.load(open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                    "manifest", "DEPOSIT.json"), encoding="utf-8"))
_decl = SURF.get("02_cover_declarations/DECLARATIONS.md", "")
check("§2 M2: the repository identifiers are now the real, archived ones (concept DOI + URL)",
      _dep["concept_doi"] in _decl and _dep["repository_url"] in _decl
      and "provided prior to publication" not in _decl)
