...
# ------------------------------------------------------------------------------------------------
# Step 5b: harmonise + MR battery for every edge with an extracted outcome file.
# Reads <exp>.clumped.tsv (exposure) + <exp>__<out>.outcome.tsv (outcome).
# Run (PowerShell): Rscript 06_mr.R
suppressMessages({library(TwoSampleMR)})
# The weighted-median p-value is bootstrapped; seed it so the network table is fully reproducible.
# Only wm_p depends on this — every point estimate, IVW/Egger p, Steiger call and Q is deterministic.
set.seed(20260718)
BASE <- P4_BASE
...
write.csv(res, file.path(RES,"forward_local_edges.csv"), row.names=FALSE)
cat("\nWrote", file.path(RES,"forward_local_edges.csv"), "with", nrow(res), "edges\n")
