...
# ------------------------------------------------------------------------------------------------
# Step 23: TPMI forward MR network (RF -> CVD/kidney), EAS 2nd cohort.
# Reads <exp>.tpmi.clumped.tsv + harmonised_tpmi/<exp>__<out>.outcome.tsv. Run: Rscript 23_tpmi_mr.R
suppressMessages({library(TwoSampleMR)})
BASE  <- P4_BASE
INSTR <- file.path(BASE,"data/instruments"); HARM <- file.path(BASE,"data/harmonised_tpmi")
RES   <- file.path(BASE,"results")
EXPS <- c("BMI","SBP","LDL","HDL","TC","TG","HbA1c"); OUTS <- c("CAD","HF","Stroke","T2D","CKD")
...
rows <- list()
for (exp in EXPS) for (out in OUTS) {
  ef <- file.path(INSTR, paste0(exp,".tpmi.clumped.tsv")); of <- file.path(HARM, paste0(exp,"__",out,".outcome.tsv"))
  ...
  if (nrow(h)<3) { cat(sprintf("%-6s->%-6s : <3 SNPs (%d)\n",exp,out,nrow(h))); next }
  ...
  rows[[key]] <- data.frame(exposure=exp, outcome=out, nsnp=gv(m,"Inverse variance weighted","nsnp"),
    ...
    steiger_correct=st$correct_causal_direction[1], stringsAsFactors=FALSE)
  cat(sprintf("%-6s->%-6s nSNP=%3d IVW b=%+.4f p=%.2e St=%s\n", exp,out,
      rows[[key]]$nsnp, rows[[key]]$ivw_b, rows[[key]]$ivw_p, rows[[key]]$steiger_correct))
}
res <- do.call(rbind, rows)
write.csv(res, file.path(RES,"network_tpmi_edges.csv"), row.names=FALSE)
cat("\nWrote network_tpmi_edges.csv with", nrow(res), "edges\n")
