...
# ------------------------------------------------------------------------------------------------
# Step 41: MR for the CAC stage-3 node edges (X->CAC and CAC->Y), same estimators as 06_mr.R.
# CAC instruments = CAC.clumped.tsv; other exposures = <exp>.clumped.tsv. Outcome files in harmonised_cac/.
# Run: Rscript 41_cac_mr.R  ->  results/network_cac.csv
suppressMessages({library(TwoSampleMR)})
set.seed(20260718)
BASE  <- P4_BASE
INSTR <- file.path(BASE, "data/instruments"); HARM <- file.path(BASE, "data/harmonised_cac")
...
write.csv(res, file.path(RES, "network_cac.csv"), row.names=FALSE)
cat("\nWrote results/network_cac.csv with", nrow(res), "CAC edges\n")
